...
const Blog = ({ store }) => {
  ...
  const STEP = 6;
  const blog = store;
  const [items, setItems] = useState(blog?.slice(0, STEP));

  const loadMore = () => {
    setItems([...items, ...blog?.slice(items?.length, items?.length + STEP)]);
  }
    ...
    <>
      ...
      <div className="section mt-5">
        <Container>
          ...
          {(items?.length < blog?.length) &&
            <Button onClick={() => loadMore()} className="bg-[#fff] max-w-max ml-3 border-none capitalize outline-none shadow1 mt-[20px] mr-0 mb-[40px] rounded-[4px] pt-[5px] pb-[5px] pl-[30px] pr-[30px] text-black text-[17px] ">
              {t("more2")}
            </Button>
          }
        </Container>
